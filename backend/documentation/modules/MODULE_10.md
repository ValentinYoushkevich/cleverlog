...
|---|----------|---------------|--------|
| 1 | Отчёт по пользователю | `GET /api/reports/user?date_from=2025-06-01&date_to=2025-06-30` → `rows`, `totals` | ✅ Пройдено |
| 2 | User видит только свои данные | С User-cookie без `user_id` → только его записи | ✅ Пройдено |
| 3 | Admin смотрит чужой | `?user_id=<uuid>` с Admin-cookie → данные указанного пользователя | ✅ Пройдено |
| 4 | Блок норма/факт при полном месяце | `?date_from=2025-06-01&date_to=2025-06-30` → `norm_block` содержит `norm`, `fact`, `deviation` | ✅ Пройдено |
| 5 | Блок норма/факт при неполном периоде | `?date_from=2025-06-01&date_to=2025-06-15` → `norm_block: null` | ✅ Пройдено |
| 6 | Отчёт по проекту | `GET /api/reports/project?project_id=<uuid>` → строки с сотрудниками, итоги по пользователям | ✅ Пройдено |
| 7 | Отчёт по проекту для User | → `403 FORBIDDEN` | ✅ Пройдено |
| 8 | Сводный отчёт | `GET /api/reports/monthly-summary?year=2025&month=6` → `rows` с `by_project`, `absence_hours`, `fact_hours`, `is_on_norm` | ✅ Пройдено |
| 9 | Незаполнившие | `GET /api/reports/unlogged?year=2025&month=6` → список пользователей с `unlogged_count > 0` | ✅ Пройдено |
| 10 | Экспорт пользователя | `GET /api/reports/user/export` → скачивается `.xlsx` файл с корректными колонками | ✅ Пройдено |
| 11 | Экспорт сводного | `.xlsx` с динамическими колонками по проектам + цветовая подсветка строк | ✅ Пройдено |
| 12 | Экспорт незаполнивших | `.xlsx` с датами через запятую в колонке `Даты` | ✅ Пройдено |
| 13 | Фильтрация по типу | `?type=vacation` → только отсутствия vacation в отчёте пользователя | ✅ Пройдено |
| 14 | Фильтрация по Task Number | `?task_number=TASK-1` → только совпадения | ✅ Пройдено |
