...
|---|----------|---------------|
| 1 | Создание диапазоном на рабочие дни | `POST /api/absences` `{ date_from: "2025-06-02", date_to: "2025-06-06" }` → `201`, созданы 5 записей (пн–пт) |
| 2 | Выходные автоматически пропускаются | Диапазон пн–вс → создаются только 5 рабочих дней; в ответе `skipped.weekends` содержит сб и вс |
| 3 | Дни с Work Logs пропускаются | Создать Work Log на среду → Absence на пн–пт → среда в `skipped.work_logs`, остальные созданы |
| 4 | Все дни заняты → ошибка | Диапазон только из выходных → `400 NO_WORKING_DAYS` |
| 5 | User видит только свои | `GET /api/absences` с User-cookie → только его записи |
| 6 | Admin видит все | `GET /api/absences?user_id=<uuid>` → записи любого пользователя |
| 7 | Редактирование: перенос на рабочий | `PATCH /api/absences/:id` `{ "date": "2025-06-09" }` (понедельник) → `200` |
| 8 | Запрет переноса на выходной | `PATCH` с датой воскресенья → `400 WEEKEND_DATE` |
| 9 | Запрет переноса на день с Work Log | `PATCH` с датой где есть Work Log → `400 WORK_LOG_CONFLICT` |
| 10 | Изменение длительности | `PATCH` `{ "duration": "4h" }` → `duration_days = 0.5`, `duration_hours = 4` |
| 11 | Удаление | `DELETE /api/absences/:id` → `200`; запись исчезает из БД |
| 12 | User не удаляет чужое | → `403 FORBIDDEN` |
| 13 | Закрытый месяц блокирует User | → `403 MONTH_CLOSED` |
| 14 | Admin работает в закрытом месяце | → `201` / `200` |
| 15 | Audit log пишется | `SELECT * FROM audit_logs WHERE entity_type='absence';` → записи есть |
