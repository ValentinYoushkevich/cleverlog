...
|---|----------|---------------|
| 1 | Все роуты из матрицы покрыты | Пройти по каждой строке матрицы с User и Admin cookie |
| 2 | Сценарий 1 проходит полностью | User не получает 403 там, где должен работать |
| 3 | Сценарий 2 проходит полностью | Admin имеет доступ ко всем эндпоинтам |
| 4 | Сценарий 3 (закрытый месяц) | User заблокирован, Admin нет |
| 5 | Сценарий 4 (inactive) | Inactive не может войти; его данные доступны Admin |
| 6 | Сценарий 5 (изоляция) | User видит только свои данные во всех эндпоинтах |
| 7 | errorHandler ловит все ошибки | Выбросить ошибку в любом сервисе → 500 с JSON (не HTML stack trace) |
| 8 | Нет незащищённых Admin-роутов | Перебрать все POST/PATCH/DELETE с User-cookie → только ожидаемые 403 |
| 9 | CORS настроен | Запрос с `http://localhost:5173` → нет ошибок CORS; с другого origin → блок |
| 10 | Cookie HttpOnly | В DevTools → cookie `token` имеет флаг HttpOnly, недоступна из JS |
