...
function fillMonthlySummarySheet(worksheet, projects, rows, totals) {
  for (const row of rows) {
    const excelRow = { user_name: row.user_name, absence_hours: row.absence_hours, fact_hours: row.fact_hours };
    for (const project of projects) { excelRow[`proj_${project.id}`] = row.by_project[project.id] || 0; }
    const addedRow = worksheet.addRow(excelRow);
    addedRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: row.is_on_norm ? 'FFD4EDDA' : 'FFFFF3CD' } };
  }
  const totalRow = { user_name: 'TOTAL', absence_hours: totals.absence_hours, fact_hours: totals.fact_hours };
  for (const project of projects) { totalRow[`proj_${project.id}`] = totals.by_project[project.id] || 0; }
...
export const ExportService = {
  ...
  async exportMonthlySummary({ year, month }) {
    const { projects, rows, totals } = await ReportService.monthlySummary({ year, month });
    const workbook = new ExcelJS.Workbook();
    ...
    fillMonthlySummarySheet(worksheet, projects, rows, totals);
    styleHeader(worksheet);
    return workbook;
  },
