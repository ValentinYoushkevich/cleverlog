...
async function resolveValidAbsenceDates(userId, date_from, date_to) {
  ...
  const [workLogDates, absenceDates] = await Promise.all([
    AbsenceRepository.findWorkLogDates(userId, workingDates),
    AbsenceRepository.findAbsenceDates(userId, workingDates),
  ]);
  ...
  return { validDates, skippedWeekend, skippedWorkLog, skippedAbsence };
}

async function buildAbsenceUpdateData(absence, id, data) {
  const updateData = {};
  if (data.date) {
    ...
    if (hasWorkLog) { throw appError(400, 'WORK_LOG_CONFLICT', 'В этот день уже есть рабочий лог'); }
...
export const AbsenceService = {
  ...
  async create({ userId, actorId, type, date_from, date_to, comment, ip }) {
    const { validDates, skippedWeekend, skippedWorkLog, skippedAbsence } =
      await resolveValidAbsenceDates(userId, date_from, date_to);
    const records = validDates.map((date) => ({
      user_id: userId, type, date, duration_days: 1, comment: comment || null,
    }));
    ...
    return {
      created: created.map((a) => ({ ...a, duration_hours: daysToHours(a.duration_days) })),
      skipped: { weekends: skippedWeekend, work_logs: skippedWorkLog, absences: skippedAbsence },
    };
  },
  ...
  async update({ id, userId, isAdmin, ip, ...data }) {
    ...
    if (!absence) { throw appError(404, 'NOT_FOUND', 'Запись не найдена'); }
    if (!isAdmin && absence.user_id !== userId) { throw appError(403, 'FORBIDDEN', 'Нет доступа'); }

    const updateData = await buildAbsenceUpdateData(absence, id, data);
    const before = {
      date: absence.date, type: absence.type, duration_days: absence.duration_days, comment: absence.comment,
    };
    const updated = await AbsenceRepository.updateById(id, updateData);
    await AuditService.log({
      actorId: userId, eventType: 'ABSENCE_UPDATED', entityType: 'absence', entityId: id,
      before, after: updateData, ip, result: 'success',
    });
