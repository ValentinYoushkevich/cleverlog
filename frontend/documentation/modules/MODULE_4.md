...
|---|----------|---------------|--------|
| 1 | Страница открывается | `/profile` → два Card без ошибок | Прошло ✅ |
| 2 | Форма профиля предзаполнена | Поля `Имя`, `Фамилия`, `Должность` содержат данные из стора | Прошло ✅ |
| 3 | Email disabled | Поле email нередактируемо | Прошло ✅ |
| 4 | Сохранение профиля | Изменить имя → «Сохранить» → `200`, сайдбар обновился | Прошло ✅ |
| 5 | Пустые обязательные поля | Очистить Имя → «Сохранить» → ошибка валидации | Прошло ✅ |
| 6 | Смена пароля успешна | Верный текущий + сильный новый → «Пароль изменён», форма сброшена | Прошло ✅ |
| 7 | Неверный текущий пароль | → `'Текущий пароль введён неверно'` | Прошло ✅ |
| 8 | Индикатор силы пароля | Вводить новый пароль → критерии загораются, Tag меняет severity | Прошло ✅ |
| 9 | Пароли не совпадают | → ошибка под полем подтверждения | Прошло ✅ || ✅ выполнено |
