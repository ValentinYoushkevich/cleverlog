...
|---|----------|---------------|
| 1 | Кнопка в шапке «Закрыть месяц» | AppLayout → кнопка видна только для Admin | ✅ выполнено |
| 2 | ConfirmDialog при закрытии | Клик → диалог подтверждения | ✅ выполнено |
| 3 | Месяц закрывается | Подтвердить → Tag «Месяц закрыт» появился в шапке, `isClosed = true` | ✅ выполнено |
| 4 | Открытие месяца | Кнопка меняется на «Открыть месяц» → подтвердить → Tag исчезает | ✅ выполнено |
| 5 | Страница уведомлений открывается | `/admin/notifications` → два Card без ошибок | ✅ выполнено |
| 6 | Глобальный ToggleSwitch | Переключить → Toast «Рассылка отключена / включена» | ✅ выполнено |
| 7 | Список пользователей | Все активные пользователи с ToggleSwitch | ✅ выполнено |
| 8 | Per-user отключение | Выключить Toggle у конкретного пользователя → сохранено | ✅ выполнено |
| 9 | Откат при ошибке | Имитировать ошибку → Toggle возвращается в прежнее состояние | ✅ выполнено |
