...
|---|----------|---------------|--------|
| 1 | Страница доступна только Admin | User → редирект (в MODULE_20) | ✅ выполнено |
| 2 | Данные загружаются | Таблица заполнена строками с Work Logs | ✅ выполнено |
| 3 | Отсутствия не отображаются | В таблице нет записей vacation/sick_leave/day_off | ✅ выполнено |
| 4 | Фильтр по проекту | Выбрать проект → только его логи | ✅ выполнено |
| 5 | Фильтр по сотруднику | Выбрать пользователя → только его строки | ✅ выполнено |
| 6 | Итоги по сотрудникам | Панель показывает суммы часов по каждому | ✅ выполнено |
| 7 | Итого по проекту | Общая сумма часов под панелью сотрудников | ✅ выполнено |
| 8 | Экспорт Excel | Скачивается `report_project.xlsx` с колонкой «Должность» | ✅ выполнено |
