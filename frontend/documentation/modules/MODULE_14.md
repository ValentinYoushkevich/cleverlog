...
|---|----------|---------------|
| 1 | Сетка рендерится корректно | Первый день в правильной колонке недели | ✅ выполнено |
| 2 | Выходные серые по умолчанию | Сб/Вс имеют `bg-gray-50` без каких-либо переопределений | ✅ выполнено |
| 3 | Клик → Popover | Клик на день → Popover с тремя опциями типа | ✅ выполнено |
| 4 | Смена типа дня | Рабочий → Праздник → сетка обновилась, день красный | ✅ выполнено |
| 5 | Индикатор переопределения | Переопределённый день — точка primary в углу | ✅ выполнено |
| 6 | Норма отображается | InputNumber заполнен текущей нормой из API | ✅ выполнено |
| 7 | Кнопка «Сохранить» disabled | Пока норма не изменена → кнопка disabled | ✅ выполнено |
| 8 | Сохранение нормы | Изменить значение → «Сохранить» → Toast + норма сохранена | ✅ выполнено |
| 9 | Навигация по месяцам | ← → переключает, переопределения корректно обновляются | ✅ выполнено |
| 10 | Изменения видны в USER-календаре | После смены праздника → в MODULE_7 день перекрашен | ✅ выполнено |
