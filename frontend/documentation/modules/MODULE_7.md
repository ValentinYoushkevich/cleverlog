...
|---|----------|---------------|--------|
| 1 | Сетка рендерится | `/calendar` → корректная сетка 7 колонок, первый день в правильной позиции | Прошло ✅ |
| 2 | Зелёные дни | Дни с Work Logs подсвечены зелёным | Прошло ✅ |
| 3 | Синие дни | Дни с Absence (без Work Logs) — синие | Прошло ✅ |
| 4 | Красные дни | Прошедшие рабочие дни без записей — красные | Прошло ✅ |
| 5 | Серые дни | Выходные — светло-серые | Прошло ✅ |
| 6 | Норма/Факт/Отклонение | В шапке корректные значения из API | Прошло ✅ |
| 7 | Навигация по месяцам | Кнопки ← → меняют месяц, данные перезагружаются | Прошло ✅ |
| 8 | Клик на день → Drawer | Drawer открывается с записями за день | Прошло ✅ |
| 9 | Удаление лога из Drawer | ConfirmDialog → подтвердить → лог удалён, день перекрасился | Прошло ✅ |
| 10 | Закрытый месяц | `isClosed = true`, User → кнопки в Drawer disabled/скрыты | Прошло ✅ |
| 11 | Сегодняшний день | Число выделено цветом primary | Прошло ✅ |
