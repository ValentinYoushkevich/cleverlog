...
| 2 | Список загружается | Записи аудита из всех предыдущих действий видны | ✅ выполнено |
| 3 | Фильтр event_type | Select заполнен вариантами из `/audit-logs/filter-options` | ✅ выполнено |
| 4 | Фильтр по типу события | Выбрать `LOGIN` → только записи входа | ✅ выполнено |
| 5 | Фильтр по result | `failure` → неуспешные попытки | ✅ выполнено |
| 6 | Поиск | Ввести `WORK_LOG` → строки содержащие это значение | ✅ выполнено |
| 7 | Монospace для event_type | Ячейка события отображается в моно-шрифте на серем фоне | ✅ выполнено |
| 8 | Кнопка «Детали» | Клик → Dialog с before/after JSON | ✅ выполнено |
| 9 | before красным, after зелёным | Блоки JSON с цветными фонами | ✅ выполнено |
| 10 | Кнопка disabled без данных | Записи без before/after → кнопка eye disabled | ✅ выполнено |
| 11 | Lazy pagination | Смена страницы → новый запрос к API | ✅ выполнено |
| 12 | Экспорт Excel | Скачивается `audit_log.xlsx` с применёнными фильтрами | ✅ выполнено |
