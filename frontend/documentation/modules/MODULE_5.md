...
|---|----------|---------------|
| 1 | Страница открывается | `/work-logs` → таблица, кнопка «Добавить лог» |
| 2 | Список загружается | Логи из БД отображаются в таблице |
| 3 | Фильтр по проекту | Выбрать проект → только его логи |
| 4 | Фильтр по Task Number | Ввести `TASK-1` → совпадения |
| 5 | Создание лога | Заполнить форму → «Создать» → лог появился в таблице |
| 6 | Длительность `1h 30m` | Создать с `1h 30m` → в таблице `1.5 ч` |
| 7 | Предупреждение >12h | Создать логи суммарно >12h → Toast с предупреждением, лог создан |
| 8 | Обязательные поля | Пустая форма → ошибки под всеми обязательными полями |
| 9 | Редактирование | Клик по карандашу → Dialog с заполненными данными → сохранить |
| 10 | Удаление | Клик по корзине → ConfirmDialog → подтвердить → лог удалён |
| 11 | Кастомные поля | Выбрать проект с кастомными полями → поля появляются в форме |
| 12 | Закрытый месяц | `isClosed = true`, User → кнопки добавления/редактирования disabled |
