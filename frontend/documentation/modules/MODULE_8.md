...
|---|----------|---------------|
| 1 | Страница открывается | `/reports/user` → таблица, фильтры |
| 2 | Данные за текущий месяц | По умолчанию выбран текущий месяц → данные загружены |
| 3 | Блок Норма/Факт/Отклонение | Полный месяц → блок виден с корректными значениями |
| 4 | Блок скрыт при неполном периоде | Выбрать 1–15 число → блок исчезает |
| 5 | Итоги по проектам | Панель с разбивкой по проектам заполнена |
| 6 | Фильтр по типу | Выбрать «Отпуск» → только vacation-строки |
| 7 | Типы как Tag | work → зелёный «Работа»; vacation → синий «Отпуск» |
| 8 | Admin видит фильтр по пользователю | Войти как Admin → Select «Все пользователи» виден |
| 9 | User не видит фильтр по пользователю | Войти как User → фильтр скрыт |
| 10 | Экспорт Excel | Нажать «Экспорт» → скачивается `report_user.xlsx` |
