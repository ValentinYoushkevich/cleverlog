...
|---|----------|---------------|
| 1 | `/login` открывается | `http://localhost:5173/login` → AuthLayout с заглушкой LoginPage |
| 2 | Guard редиректит на `/login` | Открыть `/calendar` без `_auth_check` в localStorage → редирект |
| 3 | AppLayout рендерится | Установить `_auth_check=1` в localStorage → `/calendar` → виден сайдбар + шапка |
| 4 | Навигация работает | Клик по пунктам меню → роут меняется, RouterView обновляется |
| 5 | Admin-секция скрыта | `isAdmin = false` → секция «Администрирование» не видна в сайдбаре |
| 6 | Все страницы-заглушки созданы | Переход на любой маршрут → нет ошибок роутера |
| 7 | Fallback redirect | Перейти на `/несуществующий` → редирект на `/calendar` |
| 8 | Active-класс в сайдбаре | На `/calendar` → пункт «Календарь» имеет класс `bg-primary-50 text-primary` |
