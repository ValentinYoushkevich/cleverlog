...
|---|----------|---------------|
| 1 | Страница открывается | `/absences` → таблица без ошибок |
| 2 | Список загружается | Записи из БД отображаются с типами |
| 3 | Тип отображается как Tag | `vacation` → синий Tag «Отпуск» |
| 4 | Фильтр по типу | Выбрать «Больничный» → только sick_leave |
| 5 | Создание диапазоном | Выбрать пн–пт → создаётся 5 записей |
| 6 | Диалог пропущенных дней | Диапазон с выходными → resultDialog показывает пропущенные даты |
| 7 | Пропуск дней с Work Log | Диапазон включает день с Work Log → он в `skipped.work_logs` |
| 8 | Редактирование | Изменить тип → сохранить → обновилось в таблице |
| 9 | Запрет переноса на выходной | Редактировать дату → выходной → Toast с ошибкой |
| 10 | Удаление через ConfirmDialog | Подтвердить → запись удалена |
| 11 | Закрытый месяц | `isClosed = true`, User → кнопки disabled |
