...
|---|----------|---------------|
| 1 | Кнопка полей в таблице проектов | Каждая строка имеет иконку `pi-sliders-h` |
| 2 | Drawer открывается | Клик → Drawer с заголовком «Поля проекта: Название» |
| 3 | Список привязанных полей | Поля с типами, переключателями и кнопкой отвязки |
| 4 | Toggle is_enabled | Переключить → мгновенно сохраняется, Toast |
| 5 | Toggle is_required | Переключить → мгновенно сохраняется |
| 6 | Отвязка поля | Кнопка ✕ → поле исчезло из списка |
| 7 | Привязка нового поля | «Привязать поле» → Dialog → Select показывает только непривязанные |
| 8 | Уже привязанные не в списке | В Select не появляются уже привязанные поля |
| 9 | Привязка с is_required | Поле появляется в списке с `is_required = true` |
| 10 | Форма Work Log реагирует | В MODULE_5 выбрать этот проект → поле появляется в форме |
