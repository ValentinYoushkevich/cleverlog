...
| 2 | Таблица загружается | Строки с сотрудниками и колонками проектов |
| 3 | Динамические колонки | Колонка на каждый проект из API |
| 4 | Frozen первая колонка | Горизонтальный скролл → «Сотрудник» остаётся на месте |
| 5 | Зелёная строка | Сотрудник с факт = норма → `bg-green-50` |
| 6 | Жёлтая строка | Отклонение → `bg-yellow-50` |
| 7 | Строка TOTAL | Последняя строка «TOTAL» жирная с суммами |
| 8 | Норма в шапке | Отображается норма из API |
| 9 | Навигация по месяцам | ← → меняет месяц, данные перезагружаются |
| 10 | Экспорт Excel | Скачивается файл с цветными строками и динамическими колонками |
