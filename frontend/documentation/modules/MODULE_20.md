...
|---|----------|---------------|
| 1 | Guard работает | Открыть любой защищённый роут без авторизации → `/login` | ✅ Реализовано |
| 2 | Авторизованный редиректится с `/login` | Войти → зайти на `/login` → редирект на `/calendar` | ✅ Реализовано |
| 3 | Admin-роуты закрыты для User | User → `/dashboard` → редирект на `/calendar` | ✅ Реализовано |
| 4 | Первая загрузка: спиннер | Открыть приложение → мигает спиннер пока идёт `fetchMe` | ✅ Реализовано |
| 5 | После обновления страницы — авторизован | F5 → остаётся авторизованным (cookie жива) | ✅ Реализовано |
| 6 | После обновления страницы — Admin видит Admin-секцию | F5 → сайдбар сразу правильный | ✅ Реализовано |
| 7 | Сценарий 1 проходит полностью | Все 10 шагов без ошибок | ✅ Реализовано |
| 8 | Сценарий 2 проходит полностью | Все 12 шагов без ошибок | ✅ Реализовано |
| 9 | Сценарий 3 (инвайт) | Вся цепочка от создания до входа | ✅ Реализовано |
| 10 | Сценарий 4 (изоляция) | User не видит чужие данные нигде | ✅ Реализовано |
