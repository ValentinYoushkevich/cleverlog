...
|---|----------|---------------|
| 1 | ProjectsPage: список загружается | Все проекты видны в таблице | ✅ выполнено |
| 2 | Фильтр по статусу | Кнопки «Активный / На паузе / Закрыт» фильтруют таблицу | ✅ выполнено |
| 3 | Создание проекта | Заполнить имя → «Создать» → проект в таблице + обновился стор | ✅ выполнено |
| 4 | Редактирование статуса | Active → On Hold → Tag обновился | ✅ выполнено |
| 5 | CustomFieldsPage: список | Поля с типами отображаются | ✅ выполнено |
| 6 | Создание text-поля | Заполнить имя + тип → создаётся | ✅ выполнено |
| 7 | Создание dropdown с вариантами | Добавить 3 варианта → созданы в БД | ✅ выполнено |
| 8 | Тип нельзя изменить | При редактировании — только поле имени, Message-подсказка | ✅ выполнено |
| 9 | Soft delete | Кнопка корзины → поле показывает «Удалено», при `showDeleted=false` — скрыто | ✅ выполнено |
| 10 | Restore | «Показать удалённые» → кнопка восстановить → поле снова «Активно» | ✅ выполнено |
